...
figure的参数中，h表示当前位置，t表示顶部，b表示底部，p表示浮动。通常遵循h-t-b的顺序。单用h参数是没有效果，常用的选项ht，htbp。如果强制需要图片方法某个位置不变，可以采用float宏包，提供的[H]选项。但出现的空白页情况，则需要手动调节。
...
\LaTeX{}中引用参考文献有两种方式。通常针对内容较小的文档或参考文献较少的文档中，可以采用thebibliography环境来提供文献引用；对于文献较多的文档，则建议采用的.bib格式文件来进行参考文献引用。
...
ScienceDirect出品，免费使用，支持多平台使用（网页、Windows、Linux、Mac、iPhone、iPad），可以与Word、LibreOffice、Latex等无缝链接，文献多级目录管理，支持PDF标记功能。其特色为学术社区群组功能，形成学术小组，组内成员共享文献。但其缺点也非常明确：对中文的支持并不友好。
